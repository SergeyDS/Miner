...
class MineSweeper:
    ...
    def create_settings_win(self):
        ...
        save_btn.grid(row=3, column=0, padx=20, pady=20, columnspan=2)

    def change_settings(self, row: tk.Entry, column: tk.Entry, mines: tk.Entry):
        try:
            int(row.get()), int(column.get()), int(mines.get())
        except ValueError:
            showerror('Ошибка', 'Вы вели не правильное значение')
            return
    ...
    @staticmethod
    def get_mines_places(execlude_number: int):
        indexes = list(range(1, MineSweeper.ROW * MineSweeper.COLUMNS + 1))
        print(f'Исключаем кнопку номер {execlude_number}')
        indexes.remove(execlude_number)
        shuffle(indexes)
